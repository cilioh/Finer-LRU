...
int addtmp=0, tailtmp=0, acttmp=0, deacttmp=0, retmp=0;
int tmp[5] = {0, 0, 0, 0, 0};
int pretmp[5] = {0, 0, 0, 0, 0};
...
/*
if(a%10000 == 0)
  printk("jw: __page_cache_release [%ld] %d\n", (int)(current->pid), prev_lru);
a++;
*/
...
//  enum lru_list jw_lru;
  printk("jw: add_pagevec_lru_move_fn [%ld] %d change: %d\n", (int)(current->pid), prev_lru, addtmp);
...
/*
if(c%10000 == 0)
  printk("jw: tail_pagevec_lru_move_fn [%ld] %d change: %d\n", (int)(current->pid), prev_lru, tailtmp);
c++;
*/
...
/*
if(d%10000 == 0)
  printk("jw: activate_pagevec_lru_move_fn [%ld] %d %d change: %d\n", (int)(current->pid), pprev_lru, prev_lru, acttmp);
d++;
*/ 
...
/*
if(e%10000 == 0)
  printk("jw: deactivate_file_pagevec_lru_move_fn [%ld] %d %d change: %d\n", (int)(current->pid), pprev_lru, prev_lru, deacttmp);
e++;
*/ 
...
//        lruvec->jw_count[prev_lru]++;
    ...
    if (PageWriteback(page) || PageDirty(page)) {
      ...
      list_move_tail(&page->lru, &lruvec->lists[lru + NR_LRU_LISTS*(page->idx)]);
      //list_move_tail(&page->lru, &lruvec->lists[jw_get_lru_idx(page, lru)]);
...
/*
if(b%10000 == 0) {
  printk(KERN_CONT "jw: release_pages [%ld] %d change:%d \n", (int)(current->pid), prev_lru, retmp);
}
if(b%100000 == 0) {
  enum lru_list jw_lru;
  jw_for_each_lru(jw_lru)
  ...
  printk("");
  for_each_lru(jw_lru)
    printk(KERN_CONT "[%d] %d ", (int)jw_lru, tmp[jw_lru]);
  printk("");
  for_each_lru(jw_lru)
    printk(KERN_CONT "[%d] %d ", (int)jw_lru, pretmp[jw_lru]);
}
b++;
*/
			del_page_from_lru_list(page, lruvec, page_off_lru(page)+NR_LRU_LISTS*(page->idx));
...
{
struct timespec j_local[2];
getrawmonotonic(&j_local[0]);
  //pagevec_lru_move_fn(pvec, __pagevec_lru_add_fn, NULL);
	add_pagevec_lru_move_fn(pvec, NULL);
getrawmonotonic(&j_local[1]);
calclock(j_local, &j_t[smp_processor_id()], &j_c[smp_processor_id()]);
}
